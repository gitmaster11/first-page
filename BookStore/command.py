...
def start(update: Update, context: CallbackContext) -> None:
    # human = context.bot.get_chat_member(,update.effective_user.id)
    
    admin=1306354017
    # # context.user_data['adm'] = admin
    # if human.status == "left":
    #     update.message.reply_html('<b>Botimi zdan foydalanish uchun kanalga azo bo\'ling</b>',reply_markup = join_channel())
    ...
    elif check_user(update.effective_user.id):
...
def join_check_channel(update:Update,context:CallbackContext):
    ...
    if data=='check_channel':
        human = context.bot.get_chat_member(chat_id = chatid, user_id = userid)
        if human.status == "left":
...
def command_confirm(update:Update,context:CallbackContext):
    ...
    context.bot.send_message("1615227509",xabar,parse_mode = "HTML")
    context.bot.send_location("1615227509",context.user_data['lat'],context.user_data['long'])
...
def command_books_category(update:Update,context:CallbackContext):
    query = update.callback_query
    data = str(query.data)
    context.user_data['kitob_soni'] = data
    if data.isdigit():
        query.message.delete()
...
def command_books_count(update:Update,context:CallbackContext):
    ...
    kitob_soni = context.user_data['kitob_soni']
    if data == 'nazad':
        query.message.delete()
        xabar = f"<b>Bo'lim:{context.user_data['cat_name']}</b>"
        query.message.reply_photo(open('Images/book.jpg','rb'),caption = f"{xabar}",reply_markup= books_name_button(int(kitob_soni)), parse_mode ="HTML")
        return 'state_books'
    elif data =='brauzer':
